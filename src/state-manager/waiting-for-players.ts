...
import {StateManager, Teams} from './state-manager';
...
async function isMatchExpired(stateManager: StateManager): Promise<boolean> {
      ...
      if(stateManager.matchStartedTimestamp && stateManager.matchStartedTimestamp + 5 * 60 * 1000 < Date.now()) {
        sendMessageToMasterServer({
          type: "matchEnded",
          data: { winner: "player dont connect to the lobby" },
        });
    ...
    for(const slot of spectatorSlots) {
        if(!await findAnchor(stateManager.currentImg, slot)) {
            ...
            await runSteps([
                    ...
                    {step: "clickOccurrence", data: {anchorKey: "wait_for_payers_spectator_kick"}},
                ], stateManager);
        }
    }
}
...
async function getJoinedPlayersCountKickPlayersNotInList(stateManager: StateManager): Promise<number> {
    ...
    const occupiedSlots = (await Promise.all(
      ...
      for (const slot of occupiedSlots) {
        ...
        if(currentTeam !== playerTeam) {
            const teamKey = playerTeam === "ct" ? "wait_for_payers_move_to_ct" : "wait_for_payers_move_to_t";
            await runSteps(
                [
                { step: "click", data: { anchorKey: slotName } },
