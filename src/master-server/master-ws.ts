...
import { changeStateSchema, lobbyCodeSchema, matchEndedSchema, messagesClientSchema, registerRunnersSchema, type MessageTypeClient, type SendMessageFromMaster } from './messages.schema';

export async function message(ws: Bun.ServerWebSocket<unknown>, message: string | Buffer) {
...
const messageHandlers: Record<MessageTypeClient, (ws: Bun.ServerWebSocket<unknown>, data: unknown) => Promise<{ error: string | null; }>> = {
...
async function handleLobbyCode(ws: Bun.ServerWebSocket<unknown>, data: unknown): Promise<{ error: string | null; }> {
...
export async function sendMessageToClient(ws: Bun.ServerWebSocket<unknown>, message: SendMessageFromMaster) {
    if (!ws) {
...
async function handleMatchEnded(ws: Bun.ServerWebSocket<unknown>, data: unknown): Promise<{ error: string | null; }> {
