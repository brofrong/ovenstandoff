...
export const STANDOFF2_DOWNLOAD_URL = "https://d.apkpure.com/b/XAPK/com.axlebolt.standoff2?version=latest";

export const standoffOutputPath = (name: string) =>  path.join(dirs, 'standoff2', name);
...
export async function downloadLastVersion(url: string) {
    ...
    const filename = contentDisposition?.split("filename=").at(1)?.replaceAll(/"/g, "")?.replaceAll(" ", "_");
