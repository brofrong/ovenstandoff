...
const applyMessageHandlers = (
  ws: Bun.ServerWebSocket<unknown>,
  server: ReturnType<typeof createServerSocket<typeof wsContract, unknown>>
) => {
  ...
  server.on.matchEnded(async (data) => {
    ...
    if (data.winner === 'player dont connect to the lobby') {
      await reportMatchEnded(
        runner.matchID,
        null,
        'player dont connect to the lobby',
        runner.callbackUrl
      )
    }
  ...
  // front-end
  server.requestHandler.registerView((_data, accept, _reject) => {
    viewers.add(server)
