...
export async function setup() {
  ...
  const group = await p.group(
    {
      ...
      restHost: () =>
        p.text({
          message: 'host где размещён REST мастер сервер?',
          initialValue: config.masterServerRestHost,
        }),
      wsHost: () =>
        p.text({
          message: 'host где размещён WS мастер сервер?',
          initialValue: config.masterServerWsHost,
        }),
      secret: () =>
        p.text({ message: 'секретный ключ для мастер сервера?', initialValue: config.secret }),
      count: () =>
        p.text({ message: 'количество инстансов', initialValue: config.runners.length.toString() }),
    },
    {
  ...
  if (currentConfig.runners.length !== parseInt(group.count, 10)) {
...
async function downloadAndInstallStandoff2(runners: Runner[], config: Config) {
  ...
  console.log(await LD.install('clear', path.join(unzippedFolder, 'com.axlebolt.standoff2.apk')))
