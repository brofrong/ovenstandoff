...
export type LD = ReturnType<typeof getLd>

export function getLd(config: Config) {
  return {
    launch: (name: string) => {
    ...
    modify: async (
      ...
      console.log(`${config.ldPath}\\ldconsole.exe modify --name ${name} ${toModify.join(' ')}`)
    ...
    list2: async () => {
      ...
      for (const emulatorInfo of emulatorsInfo) {
        if (!emulatorInfo) continue

        const [index, name] = emulatorInfo.split(',')
        ret.push({ index: +index!, name: name! })
      }
    ...
    isrunning: async (name: string): Promise<boolean> => {
      const bashret = await $`${config.ldPath}\\ldconsole.exe isrunning --name ${name}`.text()
      return bashret !== 'stop'
    },
