...
it('send events', (done) => {
  ...
  const sendHandler = (data: string) => {
    ...
    expect({ event: parsedData.event, data: parsedData.data }).toEqual({ event: 'event2', data: testData });
    done();
  };
...
it('context', () => {
  ...
  const client = createClientSocket(testContract, { send: (_data) => null, close: () => null }, context);
...
it('request reject', (done) => {
  ...
  server.requestHandler.event2((data, accept, reject) => {
    expect(data).toEqual(clientTestData);
    reject('test Error');
  });
