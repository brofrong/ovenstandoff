      ...
      {activeRunners.length === 0 ? (
        <Card className="bg-card border-muted shadow-sm">
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-muted-foreground">Нет активных раннеров</p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-3">
          {activeRunners.map((runner) => (
            <Card
              key={runner.name}
              className="bg-card border-muted shadow-sm hover:shadow-md transition-shadow"
            >
                          ...
                          <div className="space-y-1">
                            {runner.team.t.length > 0 ? (
                              runner.team.t.map((player, index) => (
                                <div
                                  key={index}
                                  className="text-xs bg-orange-100 px-2 py-1 rounded"
                                >
                                  {player}
                                </div>
                              ))
