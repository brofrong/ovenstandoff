...
const _checkImgApproach = async () => {
  ...
  for (let i = 0; i < TIME_TO_CHECK; i++) {
    ...
    const _test = await sharp(img!).raw().toBuffer()
...
const checkAdbApproach = async () => {
  const config = await getConfig()
  const LD = getLd(config)
  const _ldPlayer = new LDPlayer(LD_PLAYER_NAME, LD)

  const startTime = Date.now()
  for (let i = 0; i < TIME_TO_CHECK; i++) {
    const singleFrameTime = Date.now()
    const img =
      await $`${config.ldPath}\\ldconsole.exe adb --name ${LD_PLAYER_NAME} --command "exec-out screencap -p"`.arrayBuffer()
    const _test = await sharp(img).raw().toBuffer()
