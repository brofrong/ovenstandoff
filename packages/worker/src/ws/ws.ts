...
function addEventListenerHandlers(
  client: ReturnType<typeof createClientSocket<typeof wsContract, typeof ws>>
) {
  client.on.startMatch(async (data) => {
    const { teams, runner, matchID, callbackUrl, map } = data
    const runnerToStartMatch = activeStateManagers.find((it) => it.ldPlayer.name === runner)
    if (!runnerToStartMatch) {
      console.error('Runner to start match not found:', runner)
      return
    }
    const result = await runnerToStartMatch.startCreatingLobby(
      teams,
      map,
      matchID || undefined,
  ...
  client.on.startScreenStreamCommand(async (data) => {
    const { runner } = data
    const stateManager = activeStateManagers.find((it) => it.ldPlayer.name === runner)
    if (!stateManager) {
