...
export async function findName(img: Buffer, possibleNames: string[], coordsIndex: number) {
  ...
  const croppedImage = await sharp(img)
    .extract({
    ...
    .grayscale()
    .normalise({ lower: 80, upper: 95 })
    .toBuffer()

  for (const renderedName of renderedNames) {
    try {
      const { score } = await getImageOccurrence(croppedImage, renderedName.img, {
        threshold: 0.4,
        visualize: true,
      })
...
export async function findNameMSE(img: Buffer, possibleNames: string[], coordsIndex: number) {
  ...
  for (const possibleName of possibleNames) {
    try {
      ...
      console.log(
        `MSE for ${possibleName}: ${mse.toFixed(4)}, inverted score: ${invertedScore.toFixed(4)}`
      )
    } catch (error) {
